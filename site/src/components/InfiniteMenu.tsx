    ...
    Promise.all(
      ...
      images.forEach((img, i) => {
        ...
        if (item?.title) {
          ...
          // 根据文字长度调整字号
          const maxWidth = cellSize * 0.65;
          let fontSize = 56;
          ctx.font = `bold ${fontSize}px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif`;
          
          // 缩小字号直到文字适合
          while (ctx.measureText(item.title).width > maxWidth && fontSize > 24) {
            fontSize -= 4;
            ctx.font = `bold ${fontSize}px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif`;
          }
          ...
          // 绘制标题（底部位置，避开选中时的描述文本）
          const textX = x + cellSize / 2;
          const textY = y + cellSize * 0.78;
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      {activeItem && (
        <>
          {/* 描述文本显示在顶部中央 */}
          <p className={`face-description-center ${isMoving ? 'inactive' : 'active'}`}>{activeItem.description}</p>
